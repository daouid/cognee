...
@patch(f"{ADD_ROUTER_PATH}.create_dataset", new_callable=AsyncMock) # Ensured AsyncMock
@patch(f"{ADD_ROUTER_PATH}.get_datasets_by_name", new_callable=AsyncMock) # Changed from get_dataset
@patch(f"{ADD_ROUTER_PATH}.cognee_add", new_callable=AsyncMock)
@patch(f"{ADD_ROUTER_PATH}.get_authenticated_user")
def test_add_text_to_new_dataset_auto_creates(
    mock_get_auth_user, # Corresponds to get_authenticated_user
    mock_router_cognee_add, # Corresponds to cognee_add
    mock_get_datasets_by_name, # Changed from mock_router_get_dataset
    mock_router_create_dataset, # Corresponds to create_dataset
    mock_router_get_engine, # Corresponds to get_relational_engine
    mock_user_fixture,
):
    mock_get_auth_user.return_value = mock_user_fixture
    client = TestClient(app)
    ...
    # Configure mock for get_datasets_by_name to return an empty list (dataset not found)
    mock_get_datasets_by_name.return_value = []
    ...
    # Assertions
    # 1. get_datasets_by_name was called to check if dataset exists
    mock_get_datasets_by_name.assert_called_once_with(
        dataset_names=new_dataset_name, # Updated argument name
